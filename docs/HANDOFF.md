...
src/
  components/   共通UI（Card, StatCard, Button系, IconBadge, StatusPill,
                CircularTimer, HistoryList, SessionCard …）＋ index.ts で集約export
...
### 5-5. デモ用タイマー短縮（`utils/settings.ts` + `sessionRuntime.ts`）
...
- **`FocusSessionPage` は早期 return の後に `useState` を呼んでいる**（Rules of Hooks 違反）。
  config が固定なので実害は出ていないが、**安易にリファクタすると壊れる**。触るなら慎重に。
...
- **ホームの「今日の集中時間（2時間45分）」は固定値**。履歴から集計していない見せ球。
...
必須の残作業はありません（全画面実装済み・ビルド通過・デプロイ自動化済み）。改善するなら：

- 実機（iPhone 実物）での最終確認。特に**セッション中のロック挙動**とタップ感
- ホームの「今日の集中時間」を履歴から実集計する
- 実績バッジを実データ（連続成功回数など）から判定する
